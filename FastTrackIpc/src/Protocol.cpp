...
int8_t Protocol::readInt8() {
  int8_t n = 0;
  m_transport.readBytes(reinterpret_cast<char*>(&n), 1);
  return n;
}

void Protocol::writeInt8(int8_t n) {
  m_transport.writeBytes(reinterpret_cast<char*>(&n), 1);
}
