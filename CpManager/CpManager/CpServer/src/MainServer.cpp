...
void MainServer::loadHandler(const string & handlerName) {
  ICpHandler * handler = ICpHandler::Factory::create(handlerName);
  if (!handler) {
    ostringstream msg;
...
ICpHandler * MainServer::getHandlerByName(const std::string & name) {
  scoped_lock lock(m_mutex);
...
try {
  ...
  while (true) {
    ...
    ICpHandler * handler = getHandlerByName(name);
